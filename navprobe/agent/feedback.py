...
def _execution_ok(step: "NavProbeStepState") -> bool:
    if step.results == []:
        return step.executed_action is None
    return all(bool(result.ok) for _, result in step.results)


def _action_result_refs(step: "NavProbeStepState") -> dict[str, Any]:
    return {
...
def _rgb_history_obs_ids_from_results(step: "NavProbeStepState") -> list[str]:
    obs_ids: list[str] = []
