...
from navprobe.agent.vln_runner import run_vln_episode
from navprobe.config.vln_runtime import VlnRuntimeConfig
from navprobe.env.robot_rpc_env import RobotRPCEnv
from navprobe.perception.detectors.yoloworld_local import DEFAULT_YOLOWORLD_MODEL_PATH
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("NAVPROBE_MODEL", ""),
        help="OpenAI-compatible multimodal model name (or set NAVPROBE_MODEL).",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not model:
        raise ValueError("--model or NAVPROBE_MODEL is required")
