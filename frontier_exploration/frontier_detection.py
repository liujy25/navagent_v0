...
def detect_frontier_waypoints(
    ...
    return_frontiers: bool = False,
    return_filtered_explored_mask: bool = False,
):
    if DEBUG:
        import time
    ...
    frontiers, filtered_explored_mask = detect_frontiers(
        full_map,
        explored_mask,
        area_thresh,
        return_filtered_explored_mask=True,
    )
    if VISUALIZE:
        img = cv2.cvtColor(full_map * 255, cv2.COLOR_GRAY2BGR)
        img[explored_mask > 0] = (127, 127, 127)
    ...
    waypoints, waypoint_segments = frontier_waypoints(frontiers, xy, return_segments=True)
    if return_frontiers:
        if return_filtered_explored_mask:
            return waypoints, frontiers, waypoint_segments, filtered_explored_mask
        return waypoints, frontiers, waypoint_segments
    if return_filtered_explored_mask:
        return waypoints, filtered_explored_mask
    return waypoints
...
def detect_frontiers(
    ...
    if return_filtered_explored_mask:
        return frontiers, filtered_explored_mask
    return frontiers
...
def frontier_waypoints(
    frontiers: List[np.ndarray], xy: Optional[np.ndarray] = None, return_segments: bool = False
):
    ...
    for frontier in frontiers:
        if xy is None:
            waypoint, segment = get_frontier_midpoint_with_segment(frontier)
        else:
            waypoint, segment = get_closest_frontier_point_with_segment(xy, frontier)
        waypoints.append(waypoint)
        segments.append(segment)
    if return_segments:
        return np.array(waypoints), np.array(segments)
...
@njit
def get_frontier_midpoint_with_segment(frontier):
    """Return the midpoint and the local line segment that contains it."""
    line_segments = np.concatenate((frontier[:-1], frontier[1:]), axis=1).reshape(
        (-1, 2, 2)
    )
    line_lengths = np.sqrt(
...
def get_closest_frontier_point_with_segment(xy, frontier):
    """Returns the closest point on the frontier and the supporting line segment."""
    line_segments = np.concatenate([frontier[:-1], frontier[1:]], axis=1).reshape(
        (-1, 2, 2)
    )
    closest_segment, closest_point = closest_line_segment(xy, line_segments)
